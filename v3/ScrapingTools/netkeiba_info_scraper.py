...
def _initialize_chrome_driver(parameters):
...
class RaceInfoScraper(object):
    ...
    def __init__(self, parameters, db_params):
        ...
        self.driver = _initialize_chrome_driver(parameters)
    ...
    def _make_race_ids_list(self):
        ...
        """.format(
            MIN_YEAR=self.parameters['MIN_YEAR'], MAX_YEAR=self.parameters['MAX_YEAR'],
    ...
    def _make_race_id_and_target_url(self, race_calendar):
        race_id = ''.join(map(lambda x: self._get_num_str(x), race_calendar))
    ...
    def _extract_race_master_info(self, race_id):
        ...
        starting_time = re.search(
            '(.*)発走', race_data_01.split('/')[0]).group(1)
        race_coure = race_data_01.split('/')[1]
        race_weather = re.search(
            '天候:(.*)', race_data_01.split('/')[2]).group(1)
        race_condition = re.search(
            '馬場:(.*)', race_data_01.split('/')[3]).group(1)
    ...
    def _extract_race_table_info(self, race_id):
        ...
        for row in range(len(table_element_list)):
            bracket_num = int(table_element_list[
                              row].find_element_by_xpath('td[1]').text)
            horse_num = int(table_element_list[
                            row].find_element_by_xpath('td[2]').text)
            horse_name = table_element_list[
                row].find_element_by_xpath('td[4]').text
            href_to_horse = table_element_list[row].find_element_by_xpath(
                'td[4]/div/div/span/a').get_attribute("href")
            horse_age = int(re.sub("\\D", "", table_element_list[
                            row].find_element_by_xpath('td[5]').text))
            horse_sex = re.match('[0-9a-zA-Zあ-んア-ン一-鿐]', table_element_list[
                                 row].find_element_by_xpath('td[5]').text).group()

            weight_penalty = table_element_list[
                row].find_element_by_xpath('td[6]').text
            weight_penalty = float(
                weight_penalty) if weight_penalty != '' else ''

            jockey_name = table_element_list[
                row].find_element_by_xpath('td[7]').text
            href_to_jockey = table_element_list[
                row].find_element_by_xpath('td[7]/a').get_attribute("href")
            owner_name = table_element_list[
                row].find_element_by_xpath('td[8]/a').text
            href_to_owner = table_element_list[row].find_element_by_xpath(
                'td[8]/a').get_attribute("href")

            try:
                horse_weight_info = table_element_list[
                    row].find_element_by_xpath('td[9]').text
                horse_weight, horse_weight_increment = self._get_horse_weight_and_increment_one(
                    horse_weight_info)
            except ValueError:
                horse_weight, horse_weight_increment = '', ''

            win_odds = table_element_list[
                row].find_element_by_xpath('td[10]').text
            popularity_order = table_element_list[
                row].find_element_by_xpath('td[11]').text
    ...
    def get_race_master_and_table_info(self):
        race_calendar_master_list = self._make_race_ids_list()
        for race_calendar in race_calendar_master_list:
            race_id, target_url = self._make_race_id_and_target_url(race_calendar)
    ...
    def _extract_race_result_info(self, race_id):
        ...
        table_element_list = self.driver.find_elements_by_class_name('HorseList')
        for row in range(len(table_element_list)):
            arrrival_order = table_element_list[
                row].find_element_by_xpath('td[1]').text
            bracket_num = table_element_list[
                row].find_element_by_xpath('td[2]').text
            horse_num = table_element_list[
                row].find_element_by_xpath('td[3]').text
            arrival_time = table_element_list[
                row].find_element_by_xpath('td[8]').text
            arrival_diff = table_element_list[
                row].find_element_by_xpath('td[9]').text
    ...
    def _extract_past_5_race_result(self, race_id):
        ...
        for row in range(table_length):
            ...
            for i in range(len(race_name_elem_list)):
                past_x = i + 1
                race_name_element = race_name_elem_list[
                    i].find_element_by_class_name('Data02')
                past_x_race_title = race_name_element.text
                past_x_race_id = int(re.sub(
                    '\\D', '', race_name_element.find_element_by_tag_name('a').get_attribute("href")))
                arrival_order = race_name_elem_list[
                    i].find_element_by_class_name('Num').text
